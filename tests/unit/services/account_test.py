...
async def test_log_user_existing_refresh_token(user, uow, redis_client, mock_hash_password, bearer_token_generator):
    ...
    pipeline.delete.assert_any_call(f"{RedisPrefix.REFRESH_TOKEN}:{refresh_token}")
    pipeline.delete.assert_any_call(f"{RedisPrefix.USER_REFRESH_TOKEN}:{str(user.id)}")
...
async def test_logout_user_success(redis_client, bearer_token_generator, uuid_generator):
    ...
    redis.get.assert_awaited_once_with(f"{RedisPrefix.USER_REFRESH_TOKEN}:{user_id}")
    pipeline.delete.assert_any_await(f"{RedisPrefix.REFRESH_TOKEN}:{refresh_token}")
    pipeline.delete.assert_any_await(f"{RedisPrefix.USER_REFRESH_TOKEN}:{user_id}")
    pipeline.delete.assert_any_await(f"{RedisPrefix.ACCESS_TOKEN}:{access_token}")
...
async def test_refresh_success(redis_client, bearer_token_generator, uuid_generator):
    ...
    redis.get.assert_awaited_once_with(f"{RedisPrefix.REFRESH_TOKEN}:{refresh_token}")
    pipeline.delete.assert_any_await(f"{RedisPrefix.REFRESH_TOKEN}:{refresh_token}")
    ...
    pipeline.set.assert_any_await(
        f"{RedisPrefix.REFRESH_TOKEN}:{new_refresh_token}", user_id, ex=REFRESH_TOKEN_EXPIRATION_SECONDS
    )
    pipeline.set.assert_any_await(
        f"{RedisPrefix.ACCESS_TOKEN}:{access_token}", user_id, ex=ACCESS_TOKEN_EXPIRATION_SECONDS
    )
    pipeline.execute.assert_awaited_once()
...
async def test_refresh_invalid_refresh_token(redis_client, bearer_token_generator):
    ...
    redis.get.assert_awaited_once_with(f"{RedisPrefix.REFRESH_TOKEN}:{not_existing_token}")
...
async def test_refresh_redis_user_not_found(redis_client, bearer_token_generator):
    ...
    redis.get.assert_awaited_once_with(f"{RedisPrefix.REFRESH_TOKEN}:{expired_token}")
