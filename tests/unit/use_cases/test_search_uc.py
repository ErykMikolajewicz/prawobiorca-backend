...
async def test_search_file_success(mock_embedding_port, mock_regulations_repo):
    ...
    search_results = [
        {"id": "doc1", "score": 0.95, "content": "relevant document"},
        {"id": "doc2", "score": 0.85, "content": "another document"},
    ]

    mock_embedding_port.embed_queries.return_value = [embedding_vector]
    mock_regulations_repo.search.return_value = search_results

    use_case = SearchFile(embedding_port=mock_embedding_port, regulations_repository=mock_regulations_repo, query=query)

    result = await use_case.execute()

    assert result == search_results
    mock_embedding_port.embed_queries.assert_awaited_once_with([query])
    mock_regulations_repo.search.assert_awaited_once_with(embedding_vector, limit=5, threshold=0.0)
