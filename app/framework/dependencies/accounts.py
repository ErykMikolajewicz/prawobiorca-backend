...
def get_create_account(account_data: AccountCreate,
                       users_unit_of_work: UsersUnitOfWork = Depends(get_users_unit_of_work)) -> CreateAccount:
    ...
    account_data = AccountData(email, password)
    return CreateAccount(users_unit_of_work, account_data)


def get_verify_account(email_token_verifier: Annotated[EmailTokenVerifier, Depends(get_email_token_verifier)],
