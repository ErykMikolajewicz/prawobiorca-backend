...
podman run -d --pod prawobiorca-backend-pod --name prawobiorca-backend-app prawobiorca-backend-dev:2
...
podman run -d --pod prawobiorca-backend-pod --name redis-prawobiorca redis:latest redis-server --databases 1
